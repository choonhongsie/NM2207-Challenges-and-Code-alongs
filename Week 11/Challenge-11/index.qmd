...
historic_state_data_url <- "https://api.covidactnow.org/v2/states.timeseries.json?apiKey=88fc80ecc12a4d89bad14ae44d8d0257"
...
ggplot(data, aes(x=state,y=population)) + geom_bar(stat="identity") + labs(x="States",y="Population") + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
```

# ii. What fraction of the population was infected ?

```{r, echo=FALSE, warning=FALSE, message=FALSE}
ggplot(data, aes(x=state,y=(data$actuals$cases/population))) + geom_bar(stat="identity") + labs(x="States",y="Infected (%)")+theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
```

# iii. What fraction of infected persons recovered ?

```{r, echo=FALSE, warning=FALSE, message=FALSE}
#mean((data$actuals$cases - data$actuals$deaths)/data$actuals$cases)

ggplot(data, aes(x=state,y=((data$actuals$cases - data$actuals$deaths)/data$actuals$cases))) + geom_bar(stat="identity") + labs(x="States",y="Infected (%)")+theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
# iv. What fraction of the population is currently vaccinated ?
```{r, echo=FALSE, warning=FALSE, message=FALSE}
#mean(data$actuals$vaccinationsCompleted/data$population, na.rm = TRUE)

ggplot(data, aes(x=state,y=(data$actuals$vaccinationsCompleted/data$population))) + geom_bar(stat="identity") + labs(x="States",y="Infected (%)")+theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
...
geom_point(show.legend=TRUE) + labs(x="Date",y="Transmission Level")+theme_bw()+ theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
